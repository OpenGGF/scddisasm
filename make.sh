...
mkdir -p "$HEADLESS_CONFIG_DIR" "$HEADLESS_CACHE_DIR" "$HEADLESS_DATA_DIR" \
	"$HEADLESS_STATE_DIR" "$HEADLESS_GLX_DIR"
...
if [[ ${SCDDISASM_HEADLESS:-0} != 1 ]]; then
	exec env \
		...
		-u LD_PRELOAD -u ORIG_LD_LIBRARY_PATH -u WINEPREFIX \
		-u WINEDLLOVERRIDES -u SteamAppId -u SteamGameId \
		-u VK_ICD_FILENAMES -u VK_DRIVER_FILES -u VK_LAYER_PATH -u VK_INSTANCE_LAYERS \
		-u __GLX_VENDOR_LIBRARY_NAME -u __NV_PRIME_RENDER_OFFLOAD \
		-u __NV_PRIME_RENDER_OFFLOAD_PROVIDER -u DRI_PRIME \
...
unset DISPLAY WAYLAND_DISPLAY WAYLAND_SOCKET XDG_SESSION_TYPE XAUTHORITY \
	...
	LD_PRELOAD ORIG_LD_LIBRARY_PATH WINEPREFIX WINEDLLOVERRIDES \
	SteamAppId SteamGameId VK_ICD_FILENAMES VK_DRIVER_FILES VK_LAYER_PATH \
	VK_INSTANCE_LAYERS __GLX_VENDOR_LIBRARY_NAME __NV_PRIME_RENDER_OFFLOAD \
	__NV_PRIME_RENDER_OFFLOAD_PROVIDER DRI_PRIME
