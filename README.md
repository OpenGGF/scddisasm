...
The next non-USA raw `BRAMMAIN.MMD` buffer-copy routine from `$FF503C` through
`$FF5049` (14 bytes) is now labeled source. It copies three longwords from
`a0` to `a1`, clears the trailing byte, and returns; the next raw routine
begins at `$FF504A`. Japan, USA, and Europe still produce byte-exact
component outputs.

The next non-USA raw `BRAMMAIN.MMD` helper from `$FF504A` through `$FF505D`
(20 bytes) is now labeled source. It calls the regional `$FF50D4` setup
helper, loads `$4000` into `d1`, points at the following `SONICCD__00` data at
...
The next non-USA raw `BRAMMAIN.MMD` `$FFFFCD30` table-upload helper from
`$FF507E` through `$FF508F` (18 bytes) is now labeled source. It calls
`$FF50D4`, loads `$4000` into `d1`, selects `$FFFFCD30`, calls `$FF5158`, and
...
The next non-USA raw `BRAMMAIN.MMD` status helper from `$FF50E6` through
`$FF5107` (34 bytes) is now labeled source. It loads the `$4(a1)` status word,
checks the `$10(a1)` and `$2(a1)` fields, conditionally increments `d0`, and
returns; the following raw routine begins at `$FF5108`. Japan, USA, and Europe
still produce byte-exact component outputs.

The next non-USA raw `BRAMMAIN.MMD` table-comparison helper from `$FF5108`
through `$FF5133` (44 bytes) is now labeled source. It saves `d0/a1`, compares
the four fields at `$FF3C6C` against `(a0)`, `$4(a0)`, `$8(a0)`, and `$A(a0)`,
restores the registers on every path, and returns; the following raw routine
begins at `$FF5134`. Japan, USA, and Europe still produce byte-exact component
outputs.

The next non-USA raw `BRAMMAIN.MMD` table-comparison helper from `$FF5134`
through `$FF5157` (36 bytes) is now labeled source. It saves `d0/a1`, compares
the two longwords and one byte at `$FF3C6C` against `(a0)`, `$4(a0)`, and
`$8(a0)`, restores the registers on every path, and returns; the following raw
...
The next non-USA raw `BRAMMAIN.MMD` record-animation/update helper from
`$FF532C` through `$FF53A5` (122 bytes) is now labeled source. It computes the
active record span, clamps the count to `$100`, updates the `$8(a1)` field,
copies active four-word blocks from `$200044`, fills remaining blocks with
zeroes, and applies the final `$10(a1)`/`$6(a1)` status update; the following
...
The next non-USA raw `BRAMMAIN.MMD` animation-table helper from `$FF53E4`
through `$FF5445` (98 bytes) is now labeled source. It clears the record
counters, selects the active record buffer, emits twelve entries while
skipping the selected record, and calls the `$FF5158` upload/conversion
helper; the following 12-byte space table is labeled at `$FF5446`, and the
next raw routine begins at `$FF5452`. Japan, USA, and Europe still produce
byte-exact component outputs.

The next non-USA raw `BRAMMAIN.MMD` animation-table wrapper block from
`$FF5452` through `$FF5491` (64 bytes) is now labeled source as three small
routines. They set the upload mode, select the table at `$FF5446`, and call
...
The next non-USA raw `BRAMMAIN.MMD` reverse state-advance routine from
`$FF5516` through `$FF556B` (86 bytes) is now labeled source. It decrements
and wraps the animation phase, advances the `$C(a1)` position backwards
through its timing window while preserving the interrupt state, clamps the
final position, and invokes the `$FF55B6` update helper before the VDP wait;
...
The next non-USA raw `BRAMMAIN.MMD` state-update routine from `$FF55DC`
through `$FF560F` (52 bytes) is now labeled source. It updates the active
record position, copies the timing counter, invokes the record-buffer update
path, clamps the selected index, and stores the resulting record index; the
...
The next non-USA raw `BRAMMAIN.MMD` routines from `$FF5692` through `$FF5749`
(184 bytes) are now labeled source. This covers the dispatcher's three
callees, their shared status/update calls, and the CD16/CD17 selector helpers;
the following raw routine begins at `$FF574A`. Japan, USA, and Europe still
produce byte-exact component outputs.

The next non-USA raw `BRAMMAIN.MMD` object-state dispatcher and inline handler
table from `$FF574A` through `$FF5771` (40 bytes) are now labeled source. The
...
The next non-USA raw `BRAMMAIN.MMD` state-selection handler from `$FF57BC`
through `$FF57F7` (60 bytes) is now labeled source. It checks the
region/status flags, selects the object state byte, clears the status latch,
and calls the shared update routine at `$FF5828`; the following raw handler
begins at `$FF57F8`. Japan, USA, and Europe still produce byte-exact component
outputs.

The next non-USA raw `BRAMMAIN.MMD` state initializer from `$FF57F8` through
`$FF5827` (48 bytes) is now labeled source. It sets the object state and
flags, clears the required object fields, copies the existing coordinate, and
installs the fixed state values before returning; the following raw routine
begins at `$FF5828`. Japan, USA, and Europe still produce byte-exact component
outputs.

The next non-USA raw `BRAMMAIN.MMD` coordinate-check helper from `$FF5828`
through `$FF5839` (18 bytes) is now labeled source. It calls the shared
helper at `$FF5936`, compares the two object coordinate fields, and returns;
the following raw routine begins at `$FF583A`. Japan, USA, and Europe still
produce byte-exact component outputs.

The next non-USA raw `BRAMMAIN.MMD` coordinate-storage handler from
`$FF583A` through `$FF5859` (32 bytes) is now labeled source. It raises the
status latch, sets the object state and flags, clears the object field, stores
the two coordinate values, and returns; the following raw routine begins at
`$FF585A`. Japan, USA, and Europe still produce byte-exact component outputs.

The next non-USA raw `BRAMMAIN.MMD` object-state handler from `$FF585A`
through `$FF58B5` (92 bytes) is now labeled source. It checks and updates the
object subtype, adjusts the horizontal state value, sets or clears the object
flags, and calls the shared helper at `$FF6266`; the following raw routine
...
The next non-USA raw `BRAMMAIN.MMD` handler from `$FF5F0E` through `$FF5F89`
(124 bytes) is now labeled source. It dispatches on `$CD2D`, initializes state
4, computes and stores the two coordinate components through the fixed-point
helper, and returns; the following handler begins at `$FF5F8A` and remains
explicit source bytes pending semantic decomposition. Japan, USA, and Europe
still produce byte-exact component outputs.

The next non-USA raw `BRAMMAIN.MMD` handler from `$FF5F8A` through `$FF5F99`
(16 bytes) is now labeled source. It sets state 6, negates the two stored
coordinate components, and returns; the following handler begins at `$FF5F9A`
...
The next non-USA raw `BRAMMAIN.MMD` handler from `$FF5FF8` through `$FF6031`
(58 bytes) is now labeled source. It increments the object-use counter,
performs the four horizontal and vertical visibility checks, and advances the
object state when the object is outside the active bounds; the following
handler begins at `$FF6032` and remains explicit source bytes pending semantic
...
The next non-USA raw `BRAMMAIN.MMD` handler from `$FF6050` through `$FF6063`
(20 bytes) is now labeled source. It checks the pending object count, returns
through the common object path when needed, and otherwise resets the object to
state 2; the following handler begins at `$FF6064` and remains explicit source
bytes pending semantic decomposition. Japan, USA, and Europe still produce
byte-exact component outputs.

The next non-USA raw `BRAMMAIN.MMD` handler from `$FF6064` through `$FF6099`
(54 bytes) is now labeled source. It performs the common object update,
waits for the horizontal coordinate to reach its target, initializes the
vertical movement field, and applies its direction correction; the following
handler begins at `$FF609A` and remains explicit source bytes pending semantic
decomposition. Japan, USA, and Europe still produce byte-exact component
outputs.

The next non-USA raw `BRAMMAIN.MMD` handler from `$FF609A` through `$FF60BD`
(36 bytes) is now labeled source. It compares the target coordinate, advances
`$CD2E` and transitions the object to state 14 when the coordinate is reached;
otherwise it runs the common object path. The following handler begins at
`$FF60BE` and remains explicit source bytes pending semantic decomposition.
Japan, USA, and Europe still produce byte-exact component outputs.

The next non-USA raw `BRAMMAIN.MMD` handler from `$FF60BE` through `$FF60D1`
(20 bytes) is now labeled source. It checks the pending object count, returns
through the common object path when needed, and otherwise resets the object to
state 2; the following data table begins at `$FF60D2` and remains explicit
source bytes pending semantic decomposition. Japan, USA, and Europe still
produce byte-exact component outputs.

The next non-USA `BRAMMAIN.MMD` block from `$FF60D2` through `$FF6127` (86
bytes) is now labeled source. It preserves the object-data header, dispatches
on the object state through its local offset table, and initializes the state-0
object fields before the following handler begins at `$FF6128`, which remains
explicit source bytes pending semantic decomposition. Japan, USA, and Europe
still produce byte-exact component outputs.
...
The next non-USA `BRAMMAIN.MMD` table-upload routine from `$FF64AA` through
`$FF6503` (90 bytes) is now labeled source. It selects a regional table record,
invokes the still-raw upload helpers at `$FF6520` and `$FF6582`, and uploads the
...
update routine. The following data block begins at `$FF6816` and is covered
below. Japan, USA, and Europe still produce byte-exact component outputs.

The next non-USA `BRAMMAIN.MMD` data block from `$FF6816` through `$FF687D`
(104 bytes) is now explicit structured `dc.l` source, preserving its
contiguous data layout. The following data begins at `$FF687E` and remains
explicit source bytes pending semantic decomposition. Japan, USA, and Europe
still produce byte-exact component outputs.

The next non-USA `BRAMMAIN.MMD` data block from `$FF687E` through `$FF68FD`
(128 bytes) is now explicit structured `dc.l` source, preserving its
contiguous table layout. The following data begins at `$FF68FE` and remains
explicit source bytes pending semantic decomposition. Japan, USA, and Europe
still produce byte-exact component outputs.

The next non-USA `BRAMMAIN.MMD` data block from `$FF68FE` through `$FF697D`
(128 bytes) is now explicit structured `dc.l` source, preserving its
contiguous table layout. The following data begins at `$FF697E` and remains
explicit source bytes pending semantic decomposition. Japan, USA, and Europe
still produce byte-exact component outputs.

The next non-USA `BRAMMAIN.MMD` data block from `$FF697E` through `$FF69FD`
(128 bytes) is now explicit structured `dc.l` source, preserving its
contiguous table layout. The following data begins at `$FF69FE` and remains
explicit source bytes pending semantic decomposition. Japan, USA, and Europe
still produce byte-exact component outputs.

The next non-USA `BRAMMAIN.MMD` data block from `$FF69FE` through `$FF6A7D`
(128 bytes) is now explicit structured `dc.l` source, preserving its
contiguous table layout. The following data begins at `$FF6A7E` and remains
explicit source bytes pending semantic decomposition. Japan, USA, and Europe
still produce byte-exact component outputs.

The next non-USA `BRAMMAIN.MMD` data block from `$FF6A7E` through `$FF6AFD`
(128 bytes) is now explicit structured `dc.l` source, preserving its
contiguous table layout. The following data begins at `$FF6AFE` and remains
explicit source bytes pending semantic decomposition. Japan, USA, and Europe
still produce byte-exact component outputs.

The next non-USA `BRAMMAIN.MMD` data block from `$FF6AFE` through `$FF6B7D`
(128 bytes) is now explicit structured `dc.l` source, preserving its
contiguous table layout. The following data begins at `$FF6B7E` and remains
explicit source bytes pending semantic decomposition. Japan, USA, and Europe
still produce byte-exact component outputs.
...
The next non-USA `BRAMMAIN.MMD` data block from `$FF6BFE` through `$FF6C7D`
(128 bytes) is now explicit structured `dc.l` source, preserving its
contiguous table layout. The following data begins at `$FF6C7E` and remains
explicit source bytes pending semantic decomposition. Japan, USA, and Europe
still produce byte-exact component outputs.
...
and is covered by the preceding paragraph. Japan, USA, and Europe still
produce byte-exact component outputs.
...
following entries. Japan, USA, and Europe still produce byte-exact component
outputs.
...
the coordinate helper, and the index helper; the following routine begins at
`$FF5D8A` and is covered by the following paragraph. Japan, USA, and Europe
still produce byte-exact component outputs.

The next non-USA raw `BRAMMAIN.MMD` span from `$FF5D8A` through `$FF5E6F`
(230 bytes) is now labeled source. It contains four object-state transition
routines covering the coordinate/state update paths and their state changes;
the following routine begins at `$FF5E70` and is covered by the preceding
paragraph. Japan, USA, and Europe still produce byte-exact component outputs.
...
dispatcher, its three-entry handler table, two object handlers, and the
trailing five-word data table. The adjacent `$FF5C32-$FF5C4F` tables remain
explicitly boundary-labeled source data; the following handler span is covered
by the next paragraph. Japan, USA, and Europe still produce byte-exact
component outputs.
...
word data. The preceding data prefix is source-backed through `$FF5AF9`, and
the next raw routine begins at `$FF5B96`. Japan, USA, and Europe still
produce byte-exact component outputs.

The preceding non-USA raw `BRAMMAIN.MMD` data prefix from `$FF5A56` through
`$FF5AF9` (164 bytes) is now labeled source: its word header, 50-byte
sequence stream, 14-word offset table, and fourteen three-word records retain
their original boundaries and values. The object-state dispatcher follows at
`$FF5AFA`; Japan, USA, and Europe still produce byte-exact component outputs.
...
source-backed; the following data prefix is source-backed through `$FF5AF9`.
Japan, USA, and Europe still produce byte-exact component outputs.
...
coordinate data block is now source-backed through `$FF5A55`, and the
following data prefix is source-backed through `$FF5AF9`. Japan, USA, and
Europe still produce byte-exact component outputs.
...
through `$FF5A55`, and the following data prefix is source-backed through
`$FF5AF9`. Japan, USA, and Europe still produce byte-exact component outputs.
...
coordinate, dispatches through the shared object-update routines, and handles
the alternate position path before reaching the next raw span at `$FF5936`.
Japan, USA, and Europe still produce byte-exact component outputs.
