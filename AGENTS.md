...
- The Linux frontend runs Wine/Proton inside a private headless `cage` compositor by default (`HEADLESS_DESKTOP=1`, software-rendered with `WLR_RENDERER=pixman`). This prevents Windows GUI surfaces from reaching the desktop or consuming a GPU framebuffer. Keep this default for automated builds; `HEADLESS_DESKTOP=0` is an explicit interactive-diagnosis escape hatch only. If `cage` is unavailable, the build fails closed rather than opening a desktop window.
