...
- The Linux frontend runs Wine/Proton completely displayless by default (`HEADLESS_DESKTOP=1`): it unsets `DISPLAY` and `WAYLAND_DISPLAY` and sets `LIBGL_ALWAYS_SOFTWARE=1`, so no Windows GUI surface, Xwayland server, desktop window, or GPU framebuffer is created. Keep this default for automated builds; `HEADLESS_DESKTOP=0` is an explicit interactive-diagnosis escape hatch only and must not be used for normal builds. If a tool requires a display, stop and report that prerequisite rather than attaching it to the desktop or enabling a GPU-backed compositor. Some large Quartz Quadrant assembler inputs currently do not terminate displaylessly even with `/q`; that is a tool limitation, not a reason to weaken this safety invariant.
- The checked-in build frontends pass asm68k's `/q` option, and the Linux runner sets `WINEDEBUG=-all` and `PROTON_LOG=0`. Keep `/q` on direct asm68k invocations as well: it suppresses the assembler's local information window, so an automated build remains non-interactive. Direct Wine/Proton commands must also unset `DISPLAY` and `WAYLAND_DISPLAY`.
