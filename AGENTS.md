...
- The Linux frontend always runs Wine/Proton completely displayless (`HEADLESS_DESKTOP=1`): it unsets desktop/display variables at startup and again for every child tool, and sets `LIBGL_ALWAYS_SOFTWARE=1`, so no Windows GUI surface, Xwayland server, desktop window, or GPU framebuffer is created. `HEADLESS_DESKTOP=0` is rejected; the build fails closed instead of attaching a tool to the local desktop. If a tool requires a display, stop and report that prerequisite rather than weakening this safety invariant or using a compositor. Some large Quartz Quadrant assembler inputs currently do not terminate displaylessly even with `/q`; that is a tool limitation, not a reason to weaken this safety invariant.
- The checked-in build frontends pass asm68k's `/q` option, and the Linux runner forces `WINEDEBUG=-all` and `PROTON_LOG=0`. Keep `/q` on direct asm68k invocations as well: it suppresses the assembler's local information window, while the displayless environment prevents any fallback window from reaching the desktop. Direct Wine/Proton commands must use the same display-variable stripping and software-rendering settings; do not run the build through `cage`, `gamescope`, Xwayland, or a GPU-backed compositor.
