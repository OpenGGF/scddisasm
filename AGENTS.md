...
- The Linux frontend always runs Wine/Proton completely displayless (`HEADLESS_DESKTOP=1`): it launches each child with an allow-listed environment rather than inheriting the desktop/session environment, leaves display, VR-runtime, DBus, Vulkan, and GPU-offload variables absent, explicitly disables Wine's X11 and Wayland drivers (`WINEDLLOVERRIDES=winex11.drv=d;winewayland.drv=d`), points XDG config/cache/data/state/runtime, `HOME`, and temporary paths at ignored `out/headless-*` directories, and uses the fixed private `out/proton/` prefix for Proton (an external `PROTON_COMPAT_DATA_PATH` is ignored). It sets software-only WineD3D with D3D/NVAPI paths disabled and copies the host Mesa `libGLX_mesa.so.0` into ignored `out/headless-glx/libGLX_nvidia.so.0`; this satisfies Proton's optional NVIDIA-DLL probe without loading the host NVIDIA GLX library. Proton may provision standard VR DLLs in its ignored prefix, but no host OpenVR configuration or runtime is supplied. No Windows GUI surface, Xwayland server, desktop window, GPU framebuffer, NVIDIA device, DBus session, or host shader cache may be created. The runner serializes builds with `out/headless-build.lock`, terminates the wineserver associated with its isolated prefix after every child tool and again on exit, and clears a stale private server after acquiring the lock, including when a prior terminal was killed before its EXIT trap ran. A failed assembler therefore cannot leave a window or runtime resource alive while later steps run or after a terminal closes. `HEADLESS_DESKTOP=0` is rejected; the build fails closed instead of attaching a tool to the local desktop. Every child tool is also bounded by `SCDDISASM_TOOL_TIMEOUT_SECONDS` (default 120 seconds, maximum 300) and its process group is killed on timeout. If a tool requires a display or exceeds the bound, stop and report that prerequisite rather than weakening this safety invariant or using a compositor. Some large Quartz Quadrant assembler inputs currently do not terminate displaylessly even with `/q`; that is a tool limitation, not a reason to weaken this safety invariant.
- The checked-in build frontends pass asm68k's `/q` option, and the Linux runner forces `WINEDLLOVERRIDES=winex11.drv=d;winewayland.drv=d`, `WINEDEBUG=-all`, and `PROTON_LOG=0`. Keep `/q` on direct asm68k invocations as well: it suppresses the assembler's local information window, while the displayless environment prevents any fallback window from reaching the desktop. Direct Wine/Proton commands must use the same display-variable stripping, disabled display drivers, isolated XDG paths, private Mesa GLX shim, software-rendering, GPU-disable, direct-prefix runner, and timeout settings; do not run the build through `cage`, `gamescope`, Xwayland, or a GPU-backed compositor.

The Linux path was previously validated on Bazzite with Steam Proton Experimental for USA inside a private software compositor: that full build completed and all configured component comparisons matched. The current displayless default is the safe path for automated work. The deterministic ISO builder is region-aware; validate the complete data track against the matching local CHD when a disc image is available, in addition to running the component comparison script.
