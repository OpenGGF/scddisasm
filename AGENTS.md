...
- The Linux frontend runs Wine/Proton inside a private headless `cage` compositor by default (`HEADLESS_DESKTOP=1`, software-rendered with `WLR_RENDERER=pixman`). It uses a temporary private `XDG_RUNTIME_DIR`, clears inherited desktop display variables, and sets `LIBGL_ALWAYS_SOFTWARE=1`; this prevents Windows GUI surfaces from reaching the desktop or consuming a GPU framebuffer, and removes the private sockets on exit. Keep this default for automated builds; `HEADLESS_DESKTOP=0` is an explicit interactive-diagnosis escape hatch only. If `cage` or a writable runtime directory is unavailable, the build fails closed rather than opening a desktop window.
- The checked-in build frontends pass asm68k's `/q` option, and the Linux runner sets `WINEDEBUG=-all` and `PROTON_LOG=0`. Keep `/q` on direct asm68k invocations as well: it suppresses the assembler's local information window, so an automated build remains non-interactive and does not produce a popup on the desktop. Direct Wine/Proton commands must use the same headless `cage` wrapper.
