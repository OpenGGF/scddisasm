...
- The Linux frontend always runs Wine/Proton completely displayless (`HEADLESS_DESKTOP=1`): it unsets desktop/display and VR-runtime variables at startup and again for every child tool, points `XDG_CONFIG_HOME` at the ignored empty `out/headless-config/` directory, and sets software-only WineD3D with D3D/NVAPI GPU paths disabled. Proton may provision standard VR DLLs in its ignored prefix, but no host OpenVR configuration or runtime is supplied. No Windows GUI surface, Xwayland server, desktop window, or GPU framebuffer may be created. `HEADLESS_DESKTOP=0` is rejected; the build fails closed instead of attaching a tool to the local desktop. Every child tool is also bounded by `SCDDISASM_TOOL_TIMEOUT_SECONDS` (default 120 seconds, maximum 300) and its process group is killed on timeout. If a tool requires a display or exceeds the bound, stop and report that prerequisite rather than weakening this safety invariant or using a compositor. Some large Quartz Quadrant assembler inputs currently do not terminate displaylessly even with `/q`; that is a tool limitation, not a reason to weaken this safety invariant.
- The checked-in build frontends pass asm68k's `/q` option, and the Linux runner forces `WINEDEBUG=-all` and `PROTON_LOG=0`. Keep `/q` on direct asm68k invocations as well: it suppresses the assembler's local information window, while the displayless environment prevents any fallback window from reaching the desktop. Direct Wine/Proton commands must use the same display-variable stripping, isolated config, software-rendering, GPU-disable, and timeout settings; do not run the build through `cage`, `gamescope`, Xwayland, or a GPU-backed compositor.
