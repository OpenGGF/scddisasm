...
| `R32B__.MMD` | `Level/Collision Chaos/Act 2 Past.asm` | L | Legacy zone graph with semantic shared engine core, shared objects through Projector, animated tiles, and PLC graph semantically labelled | Mixed; Past palette/shared-object data, stage descriptor, PLC records, three-section layout table, and the truncated title-card mapping tail, animated-tile updater/source table, retained stage descriptor, and PLC-offset table at the start of `R32B Padding 1` structured | Zone-specific scroll/draw/load, address labels, the `R32B Padding 1` PLC/data remainder from relative `$00F2`, and the `R32B Padding 2` stream | Mixed but mostly named | J/U/E DEMO=0 | J/U/E 2026-09-02 (M161) | Structure the first retained PLC body at relative `$00F2` in `R32B Padding 1` |
