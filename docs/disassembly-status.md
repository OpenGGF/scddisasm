...
| `R32B__.MMD` | `Level/Collision Chaos/Act 2 Past.asm` | L | Legacy zone graph with semantic shared engine core, shared objects through Projector, animated tiles, and PLC graph semantically labelled | Mixed; Past palette/shared-object data, stage descriptor, PLC records, three-section layout table, and the truncated title-card mapping tail, retained animated-tile DMA updater/copy helper, and four-entry animated-tile source table at the start of `R32B Padding 1` structured | Zone-specific scroll/draw/load, address labels, the `R32B Padding 1` data remainder from relative `$00BC`, and the `R32B Padding 2` stream | Mixed but mostly named | J/U/E DEMO=0 | J/U/E 2026-09-02 (M159) | Structure the retained stage descriptor beginning at relative `$00BC` in `R32B Padding 1` |
