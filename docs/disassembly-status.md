...
| `ENDING.MMD` | `FMV/Main (Ending).asm` | L | Legacy | Opaque/mixed | 822 address labels and a large source-emitted image; command-dispatch state handlers, Word RAM transfer descriptors, and event-stream VDP-address/row-emission loops now have semantic entry points and callers alongside the startup/teardown, command-loader, VBlank transfer, Word RAM swap, joypad, name-table clear, timing, synchronization, and command-lifecycle helpers | Mixed | J/U/E | J/U/E 2026-09-04 (M382) | Recover one ending-FMV routine/data family |
