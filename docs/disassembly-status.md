...
| `R32B__.MMD` | `Level/Collision Chaos/Act 2 Past.asm` | L | Legacy zone graph with semantic shared engine core, shared objects through Projector, animated tiles, and PLC graph semantically labelled | Mixed; Past palette/shared-object data, stage descriptor, PLC records, three-section layout table, and the truncated title-card mapping tail, animated-tile updater/source table, and retained stage descriptor at the start of `R32B Padding 1` structured | Zone-specific scroll/draw/load, address labels, the `R32B Padding 1` PLC/data remainder from relative `$00CC`, and the `R32B Padding 2` stream | Mixed but mostly named | J/U/E DEMO=0 | J/U/E 2026-09-02 (M160) | Structure the retained PLC-offset table at relative `$00CC` in `R32B Padding 1` |
