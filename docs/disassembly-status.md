...
| `R32A__.MMD` | `Level/Collision Chaos/Act 2 Present.asm` | L | Legacy zone graph with semantic shared engine core, shared objects through Projector, animated tiles, PLC graph, and retained title-card/Results fragments semantically labelled | Mixed; Present palette/shared-object data, animated tile windows, stage descriptor, PLC records, three-section layout table, complete region-specific pre-chunk program/data snapshots, regional title-card and Results units, retained animated/static PLC metadata and helpers, retained Collision Chaos and orphaned stage/PLC graphs, `DEMO11A` PLC slices, Quartz break-wall tables, the first nine post-chunk Nemesis assets, both complete KamaKama mapping families, the retained shared height-profile data, and a complete label-linked Act 1 Past collision/layout graph | Zone-specific scroll/draw/load, address labels, and the `R32A Padding 2` remainder at relative `$279C-$29FF`; both JE and USA pre-chunk blocks are classified by exact range | Mixed but mostly named | J/U/E DEMO=0 | J/U/E 2026-09-02 (M143) | Classify the retained unit beginning at relative `$279C` in `R32A Padding 2` |
