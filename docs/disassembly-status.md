...
| `R31C__.MMD` | `Level/Collision Chaos/Act 1 Good Future.asm` | L | Legacy zone graph with semantic shared engine core, shared objects through Projector, animated tiles, and PLC graph semantically labelled | Mixed; Good Future palette, shared-object data, stage descriptor, and PLC records structured | Zone-specific scroll/draw/load, address labels, and padding streams | Mixed but mostly named | J/U/E DEMO=0 | J/U/E 2026-09-02 (M102) | Classify the Collision Chaos variant data tails |
...
| `R32C__.MMD` | `Level/Collision Chaos/Act 2 Good Future.asm` | L | Legacy zone graph with semantic shared engine core, shared objects through Projector, animated tiles, and PLC graph semantically labelled | Mixed; Good Future palette/shared-object data, animated tile windows, stage descriptor, shared R33 collision-geometry metadata at `$0000-$0083`, shared collision-column tail at `$0084-$01C1`, collision-row map at `$01C2-$11C1`, Act 1 Past collision-index map at `$11C2-$14E1`, Act 1 Past foreground layout at `$152A-$164B`, GHZ2/GHZ3 fallback foreground layouts at `$1668-$172F` and `$1734-$1855`, shared Wacky Workbench legacy suffix at `$185E-$1AC1`, and R32C Padding 3 shared mapping/animation and rotation-vector suffix structured | Zone-specific scroll/draw/load, address labels, and remaining R32C Padding 1 ranges | Mixed but mostly named | J/U/E DEMO=0 | J/U/E 2026-09-03 (M260) | Classify remaining R32C/R32D padding streams |
| `R32D__.MMD` | `Level/Collision Chaos/Act 2 Bad Future.asm` | L | Legacy zone graph with semantic shared engine core, shared objects through Projector, animated tiles, and PLC graph semantically labelled | Mixed; Bad Future palette/shared-object data, animated tile windows, stage descriptor, PLC records, three-section layout table, collision-column tail at `$0000-$013D`, shared collision-row map at `$013E-$113D`, Act 1 Past collision-index map at `$113E-$145D`, Act 1 Past foreground layout at `$14A6-$15C7`, GHZ2/GHZ3 fallback foreground layouts at `$15E4-$16AB` and `$16B0-$17D1`, and shared Wacky Workbench legacy suffix at `$17DA-$1A3D` structured | Zone-specific scroll/draw/load, address labels, and remaining `R32D Padding 1` ranges | Mixed but mostly named | J/U/E DEMO=0 | J/U/E 2026-09-03 (M259) | Classify remaining R32C/R32D padding streams |
