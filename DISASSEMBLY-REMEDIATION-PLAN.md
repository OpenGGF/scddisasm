...
| M75 | Complete | Recover the Collision Chaos Act 3 boss-machine core used by `R33C__.MMD` and `R33D__.MMD`: the damage-flash palette helper, camera-focus lock, six-state parent dispatcher, linked upper-child/pilot/bomb-launcher allocation, clamped player-Y tracking, and combat-entry setup. Name the shared flash timer, control flags, target Y, 16.16 vertical speed, parent/pilot link, upper-child link, attack count, and encounter phase fields; give all six dispatcher destinations stable functional entry names while leaving their later combat bodies for the next milestones. | Audit/check reports zero missing or case-mismatched includes and reduces active address-derived labels by fourteen, from 16,395 to 16,381. Fresh displayless Steam Proton Experimental builds completed for Japan, USA, and Europe, and all 133 comparisons matched in each region, including the two affected Act 3 Collision Chaos outputs. |
