...
- **Non-USA BRAMMAIN state-record table milestone:** The Japan/Europe
...
- **Non-USA BRAMMAIN data-block continuation milestone:** The Japan/Europe
  `BRAMMAIN.MMD` data block from `$FF687E` through `$FF68FD` (128 bytes) is now
  explicit structured `dc.l` source, preserving its contiguous table layout.
  The following data begins at `$FF68FE` and remains explicit source bytes
  pending semantic decomposition. Fresh guarded displayless Japan, Europe,
  and USA builds/checks report every listed target as an exact match, with zero
  differences and zero missing targets. No proprietary bytes were added.

- **Non-USA BRAMMAIN data-block continuation milestone:** The Japan/Europe
  `BRAMMAIN.MMD` data block from `$FF68FE` through `$FF697D` (128 bytes) is now
  explicit structured `dc.l` source, preserving its contiguous table layout.
  The following data begins at `$FF697E` and remains explicit source bytes
  pending semantic decomposition. Fresh guarded displayless Japan, Europe,
  and USA builds/checks report every listed target as an exact match, with zero
  differences and zero missing targets. No proprietary bytes were added.

- **Non-USA BRAMMAIN data-block continuation milestone:** The Japan/Europe
  `BRAMMAIN.MMD` data block from `$FF697E` through `$FF69FD` (128 bytes) is now
  explicit structured `dc.l` source, preserving its contiguous table layout.
  The following data begins at `$FF69FE` and remains explicit source bytes
  pending semantic decomposition. Fresh guarded displayless Japan, Europe,
  and USA builds/checks report every listed target as an exact match, with zero
  differences and zero missing targets. No proprietary bytes were added.

- **Non-USA BRAMMAIN data-block continuation milestone:** The Japan/Europe
  `BRAMMAIN.MMD` data block from `$FF69FE` through `$FF6A7D` (128 bytes) is now
  explicit structured `dc.l` source, preserving its contiguous table layout.
  The following data begins at `$FF6A7E` and remains explicit source bytes
  pending semantic decomposition. Fresh guarded displayless Japan, Europe,
  and USA builds/checks report every listed target as an exact match, with zero
  differences and zero missing targets. No proprietary bytes were added.

- **Non-USA BRAMMAIN data-block continuation milestone:** The Japan/Europe
  `BRAMMAIN.MMD` data block from `$FF6A7E` through `$FF6AFD` (128 bytes) is now
  explicit structured `dc.l` source, preserving its contiguous table layout.
  The following data begins at `$FF6AFE` and remains explicit source bytes
  pending semantic decomposition. Fresh guarded displayless Japan, Europe,
  and USA builds/checks report every listed target as an exact match, with zero
  differences and zero missing targets. No proprietary bytes were added.

- **Non-USA BRAMMAIN data-block continuation milestone:** The Japan/Europe
  `BRAMMAIN.MMD` data block from `$FF6AFE` through `$FF6B7D` (128 bytes) is now
  explicit structured `dc.l` source, preserving its contiguous table layout.
  The following data begins at `$FF6B7E` and remains explicit source bytes
  pending semantic decomposition. Fresh guarded displayless Japan, Europe,
  and USA builds/checks report every listed target as an exact match, with zero
  differences and zero missing targets. No proprietary bytes were added.

- **Non-USA BRAMMAIN data-block continuation milestone:** The Japan/Europe
  `BRAMMAIN.MMD` data block from `$FF6B7E` through `$FF6BFD` (128 bytes) is now
  explicit structured `dc.l` source, preserving its contiguous table layout.
  The following data begins at `$FF6BFE` and remains explicit source bytes
  pending semantic decomposition. Fresh guarded displayless Japan, Europe,
  and USA builds/checks report every listed target as an exact match, with zero
  differences and zero missing targets. No proprietary bytes were added.

- **Non-USA BRAMMAIN data-block continuation milestone:** The Japan/Europe
  `BRAMMAIN.MMD` data block from `$FF6BFE` through `$FF6C7D` (128 bytes) is now
  explicit structured `dc.l` source, preserving its contiguous table layout.
  The following data begins at `$FF6C7E` and remains explicit source bytes
  pending semantic decomposition. Fresh guarded displayless Japan, Europe,
  and USA builds/checks report every listed target as an exact match, with zero
  differences and zero missing targets. No proprietary bytes were added.
...
- **Non-USA BRAMMAIN state-2 handler milestone:** The Japan/Europe
  ...
  the following data table begins at `$FF60D2` and remains explicit source
  bytes pending semantic decomposition. Fresh guarded displayless Japan,
  Europe, and USA builds/checks report every listed target as an exact match,
  with zero differences and zero missing targets. No proprietary bytes were
  added.
...
- **Non-USA BRAMMAIN coordinate-transition milestone:** The Japan/Europe
  ...
  field, and applies its direction correction; the following handler begins at
  `$FF609A` and remains explicit source bytes pending semantic decomposition.
  Fresh guarded displayless Japan, Europe, and USA builds/checks report every
  listed target as an exact match, with zero differences and zero missing
  targets. No proprietary bytes were added.
...
- **Non-USA BRAMMAIN cleanup-handler milestone:** The Japan/Europe
  ...
  object-use counter, advances `$CD2E` when the counter reaches zero, and
  transitions the object to state 8; the following handler begins at `$FF6050`
  and remains explicit source bytes pending semantic decomposition. Fresh
  guarded displayless Japan, Europe, and USA builds/checks report every listed
  target as an exact match, with zero differences and zero missing targets. No
  proprietary bytes were added.

- **Non-USA BRAMMAIN visibility-handler milestone:** The Japan/Europe
  `BRAMMAIN.MMD` handler from `$FF5FF8` through `$FF6031` (58 bytes) is now
  labeled source. It increments the object-use counter, performs the four
  horizontal and vertical visibility checks, and advances the object state
  when the object is outside the active bounds; the following handler begins
  at `$FF6032` and remains explicit source bytes pending semantic decomposition.
  Fresh guarded displayless Japan, Europe, and USA builds/checks report every
  listed target as an exact match, with zero differences and zero missing
  targets. No proprietary bytes were added.

- **Non-USA BRAMMAIN state-10 handler milestone:** The Japan/Europe
  `BRAMMAIN.MMD` handler from `$FF5F9A` through `$FF5FF7` (94 bytes) is now
  labeled source. It sets state 10, selects the horizontal coordinate offset
  from `$CD17`, derives the object table address, initializes the movement
  fields, and applies the direction correction before jumping to the common
  object path; the following handler begins at `$FF5FF8` and remains explicit
...
- **Non-USA BRAMMAIN state-routine milestone:** The Japan/Europe
  `BRAMMAIN.MMD` span from `$FF5D8A` through `$FF5E6F` (230 bytes) is now
  labeled source. It contains four object-state transition routines covering
  the coordinate/state update paths and their state changes; the following
  routine begins at `$FF5E70` and is covered by the preceding entry. Fresh
  guarded displayless Japan, Europe, and USA builds/checks report every listed
  target as an exact match, with zero differences and zero missing targets. No
  proprietary bytes were added.
...
- **Non-USA BRAMMAIN routine milestone:** The Japan/Europe
  ...
  helper, and the index helper; the following routine begins at `$FF5D8A` and
  is covered by the following entry. Fresh guarded displayless Japan, Europe,
  and USA builds/checks report every listed target as an exact match, with zero
  differences and zero missing targets. No proprietary bytes were added.
...
- **Non-USA BRAMMAIN state-handler milestone:** The Japan/Europe
  ...
  inline offset tables, and two state-update handlers. The following routine
  begins at `$FF5D10` and is covered by the preceding entry. Fresh guarded
  displayless Japan, Europe, and USA builds/checks report every listed target
  as an exact match, with zero differences and zero missing targets. No
  proprietary bytes were added.
...
- **Non-USA BRAMMAIN object-handler milestone:** The Japan/Europe
  ...
  The adjacent `$FF5C32-$FF5C4F` tables remain explicitly boundary-labeled
  source data; the following handler span is covered by the next entry. Fresh
  guarded displayless Japan, Europe, and USA builds/checks report every listed
  target as an exact match, with zero differences and zero missing targets. No
  proprietary bytes were added.
...
- **Non-USA BRAMMAIN object-state handler milestone:** The Japan/Europe
  ...
  horizontal state value, sets or clears the object flags, and calls the
  shared helper at `$FF6266`; the following raw routine begins at
  `$FF58B6`. Fresh guarded displayless Japan, USA, and Europe builds/checks
  report every listed target as an exact match, with zero differences and zero
  missing targets. No proprietary bytes were added.

- **Non-USA BRAMMAIN coordinate-storage handler milestone:** The Japan/Europe
  `BRAMMAIN.MMD` handler from `$FF583A` through `$FF5859` (32 bytes) is
  now labeled source. It raises the status latch, sets the object state and
  flags, clears the object field, stores the two coordinate values, and
  returns; the following raw routine begins at `$FF585A`. Fresh guarded
...
- **Non-USA BRAMMAIN state-selection handler milestone:** The Japan/Europe
  ...
  the object state byte, clears the status latch, and calls the shared update
  routine at `$FF5828`; the following raw handler begins at `$FF57F8`. Fresh
...
- **Non-USA BRAMMAIN status/update helper milestone:** The Japan/Europe
  ...
  shared status/update calls, and the CD16/CD17 selector helpers; the
  following raw routine begins at `$FF574A`. Fresh guarded displayless Japan,
  USA, and Europe builds/checks report every listed target as an exact match,
  with zero differences and zero missing targets. No proprietary bytes were
  added.
...
- **Non-USA BRAMMAIN record/upload helper milestone:** The Japan/Europe
  ...
  skips the selected record when required, chooses either a record slot or
  the blank space table, and sends the phase-dependent tile upload through
  `$FF5158`; the following raw routine begins at `$FF55B6`. Fresh guarded
  displayless Japan, USA, and Europe builds/checks report every listed target
  as an exact match, with zero differences and zero missing targets. No
  proprietary bytes were added.
...
- **Non-USA BRAMMAIN next raw routine milestone:** The Japan/Europe
  ...
  `$FF54A4`/`$FF5492` helpers; the following raw routine begins at `$FF5492`.
  Fresh guarded displayless Japan, USA, and Europe builds/checks report every
  listed target as an exact match, with zero differences and zero missing
  targets. No proprietary bytes were added.

- **Non-USA BRAMMAIN next raw routine milestone:** The Japan/Europe
  `BRAMMAIN.MMD` animation-table helper from `$FF53E4` through `$FF5445`
  (98 bytes) is now labeled source. It clears the record counters, selects
  the active record buffer, emits twelve entries while skipping the selected
  record, and calls the `$FF5158` upload/conversion helper; the following
  12-byte space table is labeled at `$FF5446`, and the next raw routine begins
  at `$FF5452`. Fresh guarded displayless Japan, USA, and Europe builds/checks
  report every listed target as an exact match, with zero differences and zero
  missing targets. No proprietary bytes were added.

- **Non-USA BRAMMAIN next raw routine milestone:** The Japan/Europe
  `BRAMMAIN.MMD` record-selection/status helper from `$FF53A6` through
  `$FF53E3` (62 bytes) is now labeled source. It checks the shared status,
  selects the active record table, scans its `$10`-byte records through the
  `$FF5108` comparator, and stores either the matching index or `$FFFF` in
  `$10(a1)`; the following raw routine begins at `$FF53E4`. Fresh guarded
  displayless Japan, USA, and Europe builds/checks report every listed target
  as an exact match, with zero differences and zero missing targets. No
  proprietary bytes were added.
...
- **Non-USA BRAMMAIN next raw routine milestone:** The Japan/Europe
  ...
  fields, and invokes the existing `$FF5610`, `$FF532C`, and `$FF53E4`
  helpers; the following raw routine begins at `$FF5304`. Fresh guarded
  displayless Japan, USA, and Europe builds/checks report every listed target
  as an exact match, with zero differences and zero missing targets. No
  proprietary bytes were added.
...
- **Non-USA BRAMMAIN next raw routine milestone:** The Japan/Europe
  ...
  increments the character index, and converges through the shared `$FF518A`
  restore tail; the following data begins at `$FF5190`. Fresh guarded
  displayless Japan, USA, and Europe builds/checks report every listed target
  as an exact match, with zero differences and zero missing targets. No
  proprietary bytes were added.
...
- **Non-USA BRAMMAIN next raw routine milestone:** The Japan/Europe
  ...
  registers on every path, and returns; the following raw routine begins at
  `$FF5134`. Fresh guarded displayless Japan, USA, and Europe builds/checks
  report every listed target as an exact match, with zero differences and zero
  missing targets. No proprietary bytes were added.
...
- **Non-USA BRAMMAIN next raw routine milestone:** The Japan/Europe
  ...
  offset, writes each word to VDP, and returns; the next raw routine begins at
  `$FF50D4`. Fresh guarded displayless Japan, USA, and Europe builds/checks
  report every listed target as an exact match, with zero differences and zero
  missing targets. No proprietary bytes were added.

- **Non-USA BRAMMAIN next raw routine milestone:** The Japan/Europe
  `BRAMMAIN.MMD` `$FFFFCD30` table-upload helper from `$FF507E` through
  `$FF508F` (18 bytes) is now labeled source. It calls `$FF50D4`, loads
  `$4000` into `d1`, selects `$FFFFCD30`, calls `$FF5158`, and returns; the
  following raw routine begins at `$FF5090`. Fresh guarded displayless Japan,
  USA, and Europe builds/checks report every listed target as an exact match,
  with zero differences and zero missing targets. No proprietary bytes were
  added.
...
- **Non-USA BRAMMAIN next raw routine milestone:** The Japan/Europe
  ...
  `$FF5158`, and returns. Fresh guarded displayless Japan, USA, and Europe
  builds/checks report every listed target as an exact match, with zero
  differences and zero missing targets. No proprietary bytes were added.
...
- **Non-USA BRAMMAIN next raw routine milestone:** The Japan/Europe
  ...
  path, and selects the `$FF3C6C` table for the special path before
  converging at the shared copy routine beginning at `$FF503C`. Fresh guarded
  displayless Japan, USA, and Europe builds/checks report every listed target
  as an exact match, with zero differences and zero missing targets. No
  proprietary bytes were added.
...
- **Non-USA BRAMMAIN next raw routine milestone:** The Japan/Europe
  ...
  next raw routine begins at `$FF4FDC`. Fresh guarded displayless Japan, USA,
  and Europe builds/checks report every listed target as an exact match, with
  zero differences and zero missing targets. No proprietary bytes were added.

- **Non-USA BRAMMAIN next raw routine milestone:** The Japan/Europe
  `BRAMMAIN.MMD` status-processing routine from `$FF4F4A` through `$FF4FB7`
  (110 bytes) is now labeled source. It checks `$CD1F`, samples `$A1201F`,
  performs the `$92`, `$AE`, and `$AD` status actions through `$FF25B8`,
  restores `d0`, and returns `-1`, `0`, or `1`; the next raw routine begins at
  `$FF4FB8`. Fresh guarded displayless Japan, USA, and Europe builds/checks
  report every listed target as an exact match, with zero differences and
  zero missing targets. No proprietary bytes were added.
...
- **Non-USA BRAMMAIN next raw routine milestone:** The Japan/Europe
  ...
  the next raw routine begins at `$FF4F3A`. Fresh guarded displayless Japan,
  USA, and Europe builds/checks report every listed target as an exact match,
  with zero differences and zero missing targets. No proprietary bytes were
  added.
...
- **Non-USA BRAMMAIN next raw routine milestone:** The Japan/Europe
  ...
  helper call, updates the `$6`/`$4` fields, and returns `-1`, `0`, or `1`;
  the next raw routine begins at `$FF4E76`. Fresh guarded displayless Japan,
  USA, and Europe builds/checks report every listed target as an exact match,
  with zero differences and zero missing targets. No proprietary bytes were
  added.

- **Non-USA BRAMMAIN next raw routine milestone:** The Japan/Europe
  `BRAMMAIN.MMD` helper span from `$FF4DA6` through `$FF4DE5` (64 bytes) is
  now labeled source. It preserves the two related status loops, their three
  `$FF20E4` calls, the `$FF4E76`/`$FF4DE6` helper calls, and the historical
  transfers to `$FF546E`; the next raw routine begins at `$FF4DE6`. Fresh
  guarded displayless Japan, USA, and Europe builds/checks report every listed
  target as an exact match, with zero differences and zero missing targets. No
  proprietary bytes were added.

- **Non-USA BRAMMAIN next raw routine milestone:** The Japan/Europe
  `BRAMMAIN.MMD` routine from `$FF4D40` through `$FF4D4F` (16 bytes) is now
  labeled source. It loads `$CD1E`, scans five-byte records through `a0`, and
  returns on a matching record; the following table begins at `$FF4D50`. Fresh
  guarded displayless Japan, USA, and Europe builds/checks report every listed
  target as an exact match, with zero differences and zero missing targets. No
  proprietary bytes were added.

- **Non-USA BRAMMAIN next raw routine milestone:** The Japan/Europe
  `BRAMMAIN.MMD` routine from `$FF4CD4` through `$FF4D3F` (108 bytes) is now
  labeled source. It handles the command values 5, 6, and 2, runs the related
  status checks, preserves the historical stack encodings, updates `$CD1E`
  when needed, and returns; the following raw helper begins at `$FF4D40`.
  Fresh guarded displayless Japan, USA, and Europe builds/checks report every
  listed target as an exact match, with zero differences and zero missing
  targets. No proprietary bytes were added.

- **Non-USA BRAMMAIN next raw routine milestone:** The Japan/Europe
  `BRAMMAIN.MMD` routine from `$FF4C90` through `$FF4CD3` (68 bytes) is now
  labeled source. It calls `$FF4F3A`, masks the low nibble of `d0` into `d1`,
  invokes `$FF4D40`, checks the selected flag bits against offsets 1 through 4
  of `a0`, returns when none match, and branches to the following raw entry at
  `$FF4CD4` on a match. Fresh guarded displayless Japan, USA, and Europe
  builds/checks report every listed target as an exact match, with zero
  differences and zero missing targets. No proprietary bytes were added.

- **Non-USA BRAMMAIN next raw routine milestone:** The Japan/Europe
  `BRAMMAIN.MMD` routine from `$FF4C86` through `$FF4C8F` (10 bytes) is now
  labeled source. It clears `$CD2C` and `$CD29`, then returns; the following
  raw routine begins at `$FF4C90`. Fresh guarded displayless Japan, USA, and
  Europe builds/checks report every listed target as an exact match, with zero
  differences and zero missing targets. No proprietary bytes were added.

- **Non-USA BRAMMAIN next raw routine milestone:** The Japan/Europe
  `BRAMMAIN.MMD` routine from `$FF4C5E` through `$FF4C85` (40 bytes) is now
  labeled source. It calls `$FF6430`, clears `$CD26` and `$CD27`, checks
  `$FF3BEA`, selects `$CD29 = 2` or `$CD29 = 1`, and returns; the following raw
  clear-state entry begins at `$FF4C86`. Fresh guarded displayless Japan, USA,
  and Europe builds/checks report every listed target as an exact match, with
  zero differences and zero missing targets. No proprietary bytes were added.

- **Non-USA BRAMMAIN next raw routine milestone:** The Japan/Europe
  `BRAMMAIN.MMD` routine from `$FF4C52` through `$FF4C5D` (12 bytes) is now
  labeled source. It calls the clear-state entry at `$FF4C86`, restores
  `$CD14` from `$CD18`, and returns; the following raw routine begins at
  `$FF4C5E`. Fresh guarded displayless Japan, USA, and Europe builds/checks
  report every listed target as an exact match, with zero differences and zero
  missing targets. No proprietary bytes were added.

- **Non-USA BRAMMAIN next raw routine milestone:** The Japan/Europe
  `BRAMMAIN.MMD` routine from `$FF4C4A` through `$FF4C51` (8 bytes) is now
  labeled source. It calls `$FF4F4A`, returns when the result is zero, and
  branches to the following raw routine at `$FF4C52` when nonzero. Fresh
  guarded displayless Japan, USA, and Europe builds/checks report every listed
  target as an exact match, with zero differences and zero missing targets. No
  proprietary bytes were added.
...
- **Non-USA BRAMMAIN next raw routine milestone:** The Japan/Europe
  ...
  the following raw routine begins at `$FF4C38`. Fresh guarded displayless
  Japan, USA, and Europe builds/checks report every listed target as an exact
  match, with zero differences and zero missing targets. No proprietary bytes
  were added.

- **Non-USA BRAMMAIN next raw routine milestone:** The Japan/Europe
  `BRAMMAIN.MMD` routine from `$FF4BDC` through `$FF4C13` (56 bytes) is now
  labeled source. It handles the second status path, conditionally runs the
  `$009B` status action, updates the `$CD2A`, `$CD26`, and `$CD27` state bytes,
  and provides both return paths; the following raw routine begins at
  `$FF4C14`. Fresh guarded displayless Japan, USA, and Europe builds/checks
  report every listed target as an exact match, with zero differences and zero
  missing targets. No proprietary bytes were added.

- **Non-USA BRAMMAIN next raw routine milestone:** The Japan/Europe
  `BRAMMAIN.MMD` routine from `$FF4BA4` through `$FF4BDB` (56 bytes) is now
  labeled source. It handles the status-result branches, conditionally runs
  the `$009B` status action, updates the `$CD2A`, `$CD26`, and `$CD27` state
  bytes, and returns; the following raw routine begins at `$FF4BDC`. Fresh
  guarded displayless Japan, USA, and Europe builds/checks report every listed
  target as an exact match, with zero differences and zero missing targets. No
  proprietary bytes were added.
...
- **Non-USA BRAMMAIN next raw routine milestone:** The Japan/Europe
  `BRAMMAIN.MMD` routine from `$FF4B2A` through `$FF4B79` (80 bytes) is now
  labeled source. It dispatches the status result, updates the `$CD14`,
  `$CD2A`, and `$CD2B` state bytes, and returns; the following raw routine
  begins at `$FF4B7A`. Fresh guarded displayless Japan, USA, and Europe
  builds/checks report every listed target as an exact match, with zero
  differences and zero missing targets. No proprietary bytes were added.

- **Non-USA BRAMMAIN next raw routine milestone:** The Japan/Europe
  `BRAMMAIN.MMD` routine from `$FF4AA4` through `$FF4B29` (134 bytes) is now
  labeled source. It runs the status/clock update path, clamps the two
  mode-dependent byte fields, writes the updated value, and returns; the
  following raw routine begins at `$FF4B2A`. Fresh guarded displayless Japan,
  USA, and Europe builds/checks report every listed target as an exact match,
  with zero differences and zero missing targets. No proprietary bytes were
  added.

- **Non-USA BRAMMAIN next raw routine milestone:** The Japan/Europe
  `BRAMMAIN.MMD` routine from `$FF4A7A` through `$FF4AA3` (42 bytes) is now
  labeled source. It synchronizes the command-state bytes, runs the status
  handler, selects the follow-up command, and returns; the following raw
  routine begins at `$FF4AA4`. Fresh guarded displayless Japan, USA, and Europe
  builds/checks report every listed target as an exact match, with zero
  differences and zero missing targets. No proprietary bytes were added.

- **Non-USA BRAMMAIN next raw routine milestone:** The Japan/Europe
  `BRAMMAIN.MMD` routine from `$FF49B6` through `$FF4A79` (196 bytes) is now
  labeled source. It builds the alternate `$B4C0` buffer records, applies the
  fixed record identifiers and coordinate adjustments, runs both bounded
  loops, and returns; the following raw routine begins at `$FF4A7A`. Fresh
  guarded displayless Japan, USA, and Europe builds/checks report every listed
  target as an exact match, with zero differences and zero missing targets. No
  proprietary bytes were added.
...
- **Non-USA BRAMMAIN next raw routine milestone:** The Japan/Europe
  ...
  raw routine begins at `$FF495A`. Fresh guarded displayless Japan, USA, and
  Europe builds/checks report every listed target as an exact match, with zero
  differences and zero missing targets. No proprietary bytes were added.

- **Non-USA BRAMMAIN final post-table target milestone:** The Japan/Europe
  `BRAMMAIN.MMD` routine from `$FF48D8` through `$FF4929` (82 bytes), the final
  target selected by the post-table offset table, is now labeled source. It
  runs the result/status handlers, updates `$FF0F1F` and `$CD50`, restores the
  command/result state, and returns; the next raw routine begins at `$FF492A`.
  Fresh guarded displayless Japan, USA, and Europe builds/checks report every
  listed target as an exact match, with zero differences and zero missing
  targets. No proprietary bytes were added.

- **Non-USA BRAMMAIN following post-table routine milestone:** The
  Japan/Europe `BRAMMAIN.MMD` routine from `$FF48B0` through `$FF48D7` (40
  bytes), the next target selected by the post-table offset table, is now
  labeled source. It clears and checks the `$CD50/$CD51` command-result state,
  runs the completion handlers, and restores the command/result state; the next
  raw routine begins at `$FF48D8`. Fresh guarded displayless Japan, USA, and
  Europe builds/checks report every listed target as an exact match, with zero
  differences and zero missing targets. No proprietary bytes were added.

- **Non-USA BRAMMAIN following post-table routine milestone:** The
  Japan/Europe `BRAMMAIN.MMD` routine from `$FF4882` through `$FF48AF` (46
  bytes), the next target selected by the post-table offset table, is now
  labeled source. It checks the command flag, handles the direct `$CD50`
  completion path, and otherwise runs the shared handler before dispatching
  with command state C; the next table-selected routine begins at `$FF48B0`.
  Fresh guarded displayless Japan, USA, and Europe builds/checks report every
  listed target as an exact match, with zero differences and zero missing
  targets. No proprietary bytes were added.

- **Non-USA BRAMMAIN next post-table routine milestone:** The Japan/Europe
  `BRAMMAIN.MMD` routine from `$FF4870` through `$FF4881` (18 bytes), the next
  nonzero target selected by the post-table offset table, is now labeled source.
  It sets command state A, calls the shared handler at `$FF5716`, loads
  `d0 = 1`, and branches to `$FF4B7A`; the next table-selected routine begins
  at `$FF4882`. Fresh guarded displayless Japan, USA, and Europe builds/checks
  report every listed target as an exact match, with zero differences and zero
  missing targets. No proprietary bytes were added.

- **Non-USA BRAMMAIN ninth post-table routine milestone:** The Japan/Europe
  `BRAMMAIN.MMD` routine from `$FF4858` through `$FF486F` (24 bytes), the next
  nonzero target selected by the post-table offset table, is now labeled source.
  It clears and checks the `$CD50/$CD51` command-result state, then clears the
  result byte and sets command state 9 on the completion path; the next
  table-selected routine begins at `$FF4870`. Fresh guarded displayless Japan,
  USA, and Europe builds/checks report every listed target as an exact match,
  with zero differences and zero missing targets. No proprietary bytes were
  added.

- **Non-USA BRAMMAIN fifth post-table routine milestone:** The Japan/Europe
  `BRAMMAIN.MMD` routine from `$FF4830` through `$FF4857` (40 bytes), the
  fifth target selected by the post-table offset table, is now labeled source.
  It clears and checks the command-result byte, handles the completion path,
  and restores the command/result state; the next table-selected routine
  begins at `$FF4858`. Fresh guarded displayless Japan, USA, and Europe
  builds/checks report every listed target as an exact match, with zero
  differences and zero missing targets. No proprietary bytes were added.

- **Non-USA BRAMMAIN fourth post-table routine milestone:** The Japan/Europe
  `BRAMMAIN.MMD` routine from `$FF47B4` through `$FF482F` (124 bytes), the
  fourth target selected by the post-table offset table, is now labeled source.
  It checks the Backup RAM command flag and status byte, runs the shared
  handlers, resets the result state, and includes helper entry points at
  `$FF4808`, `$FF480C`, and `$FF481E`; the next table-selected routine begins
  at `$FF4830`. Fresh guarded displayless Japan, USA, and Europe builds/checks
  report every listed target as an exact match, with zero differences and zero
  missing targets. No proprietary bytes were added.
...
- **Non-USA BRAMMAIN second post-table routine milestone:** The Japan/Europe
  ...
  source. It checks and clears the command-result flag, sets command state 2,
  and returns; the next table-selected routine begins at `$FF47A2`. Fresh
  guarded displayless Japan, USA, and Europe builds/checks report every listed
  target as an exact match, with zero differences and zero missing targets.
  No proprietary bytes were added.
...
- **Non-USA BRAMMAIN final indexed-table target milestone:** The Japan/Europe
  ...
  source. It clears and updates the Backup RAM state, handles the completion
  paths, tests the result, invokes the cleanup handlers, and returns; the
  independent dispatcher begins at `$FF4732`. Fresh guarded displayless Japan,
  USA, and Europe builds/checks report every listed target as an exact match,
  with zero differences and zero missing targets. No proprietary bytes were
  added.

- **Non-USA BRAMMAIN eleventh table-target milestone:** The Japan/Europe
  `BRAMMAIN.MMD` routine from `$FF46A8` through `$FF46C9` (34 bytes), the
  eleventh target selected by the indexed table, is now labeled source. It
  checks the result flag, sets the command state, invokes the shared handler,
  and routes the result through the common dispatcher; the next table target
  begins at `$FF46CA`. Fresh guarded displayless Japan, USA, and Europe
  builds/checks report every listed target as an exact match, with zero
  differences and zero missing targets. No proprietary bytes were added.

- **Non-USA BRAMMAIN tenth table-target milestone:** The Japan/Europe
  `BRAMMAIN.MMD` routine from `$FF4686` through `$FF46A7` (34 bytes), the
  tenth target selected by the indexed table, is now labeled source. It runs
  the completion/status handlers, clears the result flag, and sets the
  follow-up command state; the next table target begins at `$FF46A8`.
  Fresh guarded displayless Japan, USA, and Europe builds/checks report every
  listed target as an exact match, with zero differences and zero missing
  targets. No proprietary bytes were added.

- **Non-USA BRAMMAIN ninth table-target milestone:** The Japan/Europe
  `BRAMMAIN.MMD` routine from `$FF4662` through `$FF4685` (36 bytes), the
  ninth target selected by the indexed table, is now labeled source. It
  checks and clears the completion flag, restores the status bytes, and
  invokes the completion handler; the next table target begins at `$FF4686`.
  Fresh guarded displayless Japan, USA, and Europe builds/checks report every
  listed target as an exact match, with zero differences and zero missing
  targets. No proprietary bytes were added.

- **Non-USA BRAMMAIN eighth table-target milestone:** The Japan/Europe
  `BRAMMAIN.MMD` routine from `$FF460E` through `$FF4661` (84 bytes), the
  eighth target selected by the indexed table, is now labeled source. It
  checks the status flag, updates the Backup RAM state and command bytes, and
  invokes the completion handlers; the next table target begins at `$FF4662`.
  Fresh guarded displayless Japan, USA, and Europe builds/checks report every
  listed target as an exact match, with zero differences and zero missing
  targets. No proprietary bytes were added.

- **Non-USA BRAMMAIN seventh table-target milestone:** The Japan/Europe
  `BRAMMAIN.MMD` routine from `$FF45EE` through `$FF460D` (32 bytes), the
  seventh target selected by the indexed table, is now labeled source. It
  checks the Backup RAM state, sets the command state for the success path,
  and enters the shared handler; the next table target begins at `$FF460E`.
  Fresh guarded displayless Japan, USA, and Europe builds/checks report every
  listed target as an exact match, with zero differences and zero missing
  targets. No proprietary bytes were added.

- **Non-USA BRAMMAIN sixth table-target milestone:** The Japan/Europe
  `BRAMMAIN.MMD` routine from `$FF452E` through `$FF45ED` (192 bytes), the
  sixth target selected by the indexed table, is now labeled source. It
  selects the result buffers, routes the success/failure status paths, and
  finalizes the command state; the next table target begins at `$FF45EE`.
  Fresh guarded displayless Japan, USA, and Europe builds/checks report every
  listed target as an exact match, with zero differences and zero missing
  targets. No proprietary bytes were added.

- **Non-USA BRAMMAIN fifth table-target milestone:** The Japan/Europe
  `BRAMMAIN.MMD` routine from `$FF4494` through `$FF452D` (154 bytes), the
  fifth target selected by the indexed table, is now labeled source. It
  dispatches the status/result paths, invokes the shared result handlers, and
  initializes the command-state branches; the next table target begins at
  `$FF452E`. Fresh guarded displayless Japan, USA, and Europe builds/checks
  report every listed target as an exact match, with zero differences and zero
  missing targets. No proprietary bytes were added.

- **Non-USA BRAMMAIN fourth table-target milestone:** The Japan/Europe
  `BRAMMAIN.MMD` routine from `$FF4468` through `$FF4493` (44 bytes), the
  fourth target selected by the indexed table, is now labeled source. It checks
  the status byte, handles the clear/status path, and sets up the command-6
  path; the next table target begins at `$FF4494`. Fresh guarded displayless
  Japan, USA, and Europe builds/checks report every listed target as an exact
  match, with zero differences and zero missing targets. No proprietary bytes
  were added.
...
- **Non-USA BRAMMAIN second table-target milestone:** The Japan/Europe
  ...
  handles the status/result path and its reset fallback; the next table target
  begins at `$FF4454`. Fresh guarded displayless Japan, USA, and Europe
  builds/checks report every listed target as an exact match, with zero
  differences and zero missing targets. No proprietary bytes were added.

- **Non-USA BRAMMAIN first table-target milestone:** The Japan/Europe
  `BRAMMAIN.MMD` routine from `$FF432E` through `$FF43E1` (180 bytes), the
  first target selected by the indexed table, is now labeled source. It handles
  the command-2 path, result/status updates, and the associated display-state
  branches; the next table target begins at `$FF43E2`. Fresh guarded
...
- **Non-USA BRAMMAIN status-clear helper milestone:** The Japan/Europe
  ...
  paths and enters the dispatch at `$FF42AC`. Fresh guarded displayless Japan,
  USA, and Europe builds/checks report every listed target as an exact match,
  with zero differences and zero missing targets. No proprietary bytes were
  added.

- **Non-USA BRAMMAIN status-selection milestone:** The Japan/Europe
  `BRAMMAIN.MMD` helper from `$FF422E` through `$FF4273` (70 bytes) is now
  labeled source. It selects status-byte paths for command states 5 and 6 and
  branches to the shared dispatch at `$FF42AC`; the next raw entry begins at
  `$FF4274`. Fresh guarded displayless Japan, USA, and Europe builds/checks
  report every listed target as an exact match, with zero differences and zero
  missing targets. No proprietary bytes were added.
...
- **Non-USA BRAMMAIN status/result milestone:** The Japan/Europe
  ...
  buffer, routes status outcomes for operations 1 and 3, and retains its
  three-word tail table; the next raw routine begins at `$FF413C`. Fresh
  guarded displayless Japan, USA, and Europe builds/checks report every listed
  target as an exact match, with zero differences and zero missing targets. No
  proprietary bytes were added.
...
- **Non-USA BRAMMAIN command-11/6/7 milestone:** The Japan/Europe
  ...
  from `$FF3F16` through `$FF3F27` (18 bytes) are now labeled source. They
  submit their command values through the regional status dispatcher; the next
  raw helper begins at `$FF3F28`. Fresh guarded displayless Japan, USA, and
  Europe builds/checks report every listed target as an exact match, with zero
  differences and zero missing targets. No proprietary bytes were added.
...
- **Non-USA BRAMMAIN command-result milestone:** The Japan/Europe
  ...
  two status paths, and returns result/error values in `d0/d1`; the following
  regional helper begins at `$FF3D44`. Fresh guarded displayless Japan, USA,
  and Europe builds/checks report every listed target as an exact match, with
  zero differences and zero missing targets. No proprietary bytes were added.

- **Non-USA BRAMMAIN status-dispatch milestone:** The Japan/Europe
  `BRAMMAIN.MMD` helper from `$FF3C90` through `$FF3CB9` (42 bytes) is now
  labeled source. It checks the status result, updates `$200028`, invokes the
  regional VDP/status routines, and reports command status through `$200021`;
  the following helper begins at `$FF3CBA`. Fresh guarded displayless Japan,
  USA, and Europe builds/checks report every listed target as an exact match,
  with zero differences and zero missing targets. No proprietary bytes were
  added.
...
- **Non-USA BRAMMAIN command-dispatch milestone:** The Japan/Europe
  ...
  through the `$FF2B94` routine table, records result/status at
  `$200021-$200024`, and clears the command byte; the routine table remains
  explicit data. Fresh guarded displayless Japan, USA, and Europe builds/checks
  report every listed target as an exact match, with zero differences and zero
  missing targets. No proprietary bytes were added.
...
- **Non-USA BRAMMAIN object-record-helper milestone:** The Japan/Europe
  `BRAMMAIN.MMD` helper from `$FF29BE` through `$FF2A03` (70 bytes) is now
  labeled source. It handles the default record format and writes transformed
  position and mapping fields into the `$B000` output stream; bit-selected
  variants begin at `$FF2A04` and `$FF2A52`. Fresh guarded displayless Japan,
  USA, and Europe builds/checks report every listed target as an exact match,
  with zero differences and zero missing targets. No proprietary bytes were
  added.
...
- **Non-USA BRAMMAIN mirrored-record-helper milestone:** The Japan/Europe
  `BRAMMAIN.MMD` helper from `$FF2A52` through `$FF2A97` (70 bytes) is now
  labeled source. It emits the mirrored record format with its alternate
  tile/attribute transformation and returns at `$FF2A96`; the next variant
  begins at `$FF2A98`. Fresh guarded displayless Japan, USA, and Europe
  builds/checks report every listed target as an exact match, with zero
  differences and zero missing targets. No proprietary bytes were added.

- **Non-USA BRAMMAIN final-record-helper milestone:** The Japan/Europe
  `BRAMMAIN.MMD` helper from `$FF2A98` through `$FF2AEB` (84 bytes) is now
  labeled source. It emits the remaining record format and returns at
  `$FF2AEA`; the following data table begins at `$FF2AEC`. Fresh guarded
  displayless Japan, USA, and Europe builds/checks report every listed target
  as an exact match, with zero differences and zero missing targets. No
  proprietary bytes were added.
...
- **Non-USA BRAMMAIN hardware-polling milestone:** The Japan/Europe
  ...
  `$A1200F` bit-7 wait directions; the setup routine beginning at `$FF2396`
  is covered by the subsequent VDP-setup milestone. Fresh guarded displayless
  Japan, USA, and Europe builds/checks report every listed target as an exact
  match, with zero differences and zero missing targets. No proprietary bytes
  were added.
...
- **R63C non-USA animation-routine milestone:** The Japanese/European R63C
  ...
  undifferentiated words. The fixed `$6A42`, `$B011`, `$6A00,$0034`,
  `$B029,$0001`, and `$6502` encodings remain explicit `dc.w` values to
  preserve the historical link layout. Fresh guarded displayless Proton builds
  and checks remain exact for Japan, USA, and Europe, with zero differences and
  zero missing targets; no proprietary bytes were added.
...
- **R43 retained CollapseFloor/PlatformChain/Pressure Elevator milestone:**
  ...
  guarded displayless Japan and Europe builds/checks compare all three bodies
  exactly; the next R43C and R43D differences both begin at byte 63991 in the
  following retained body. USA remains 133/0/0 (matches/differences/missing).
  No proprietary bytes were added, and the ignored CHD-derived comparison
  trees remain untracked.

- **R43 retained CircleSpikeBall milestone:** The non-USA R43 source now
  restores the retained CircleSpikeBall chain links, relocated sprite pointer,
  draw, and despawn links. Fresh guarded displayless Japan and Europe
  builds/checks compare the entire CircleSpikeBall body exactly; the next
  R43C and R43D differences both begin at byte 62225 in the following retained
  body. USA remains 133/0/0 (matches/differences/missing). No proprietary
...
- **R43 retained Tonbo milestone:** The non-USA R43 source now restores the
  ...
  next R43C and R43D differences both begin at byte 56169 in the following
  retained Amenbo body. USA remains 133/0/0 (matches/differences/missing).
  No proprietary bytes were added, and the ignored CHD-derived comparison
  trees remain untracked.
