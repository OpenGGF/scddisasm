...
- **Remaining R31A dependency graph:** The tracked historical source references stage chunks (54,784 bytes), blocks (3,960 bytes), foreground (290 bytes), background (24 bytes), and stage graphics (10,162 bytes), alongside scroll/drawing/loading/object-index code. Those assets and the corresponding current-architecture entry point are not yet present under `src/Level/Collision Chaos`, so the new metadata slice is intentionally not wired into a build.
