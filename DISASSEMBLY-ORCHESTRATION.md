...
- **Remaining R31A dependency graph:** The tracked historical source still requires blocks (3,960 bytes), foreground (290 bytes), background (24 bytes), and stage graphics (10,162 bytes), alongside scroll/drawing/loading/object-index code. Those assets and the corresponding current-architecture entry point are not yet present under `src/Level/Collision Chaos`, so the metadata and chunk slices are intentionally not wired into a build.
