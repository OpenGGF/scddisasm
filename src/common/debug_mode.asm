...
DebugMode:
	move.b	(p1_joy_hold).w,d0
	andi.b	#$F,d0
	bne.s	loc_206DBE
	move.l	#$4000,(debug_speed).l
	bra.s	loc_206DDE

; ------------------------------------------------------------------------------

loc_206DBE:
	addi.l	#$2000,(debug_speed).l
	cmpi.l	#$80000,(debug_speed).l
	bls.s	loc_206DDE
	move.l	#$80000,(debug_speed).l

loc_206DDE:
	move.l	(debug_speed).l,d0
	btst	#0,(p1_joy_hold).w
	beq.s	loc_206DF0
	sub.l	d0,obj.y(a0)

loc_206DF0:
	btst	#1,(p1_joy_hold).w
	beq.s	loc_206DFC
	add.l	d0,obj.y(a0)

loc_206DFC:
	btst	#2,(p1_joy_hold).w
	beq.s	loc_206E08
	sub.l	d0,obj.x(a0)

loc_206E08:
	btst	#3,(p1_joy_hold).w
	beq.s	loc_206E14
	add.l	d0,obj.x(a0)
...
loc_206E14:
	...
	move.w	obj.x(a0),d3
	jsr	GetBlock
	move.w	(a1),(debug_block).l
	lea	(DebugObjects).l,a2
	btst	#6,(p1_joy_tap).w
	beq.s	loc_206E56
	moveq	#0,d1
	move.b	(debug_object).l,d1
	addq.b	#1,d1
	cmp.b	(a2),d1
	bcs.s	loc_206E50
	move.b	#0,d1
...
loc_206E76:
	...
	move.b	$C(a2,d1.w),(debug_subtype_2).l
	move.b	$B(a2,d1.w),d0
	ori.b	#4,d0
